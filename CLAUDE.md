...
- **GPU fallback / unsupported-arch hide:** some GPUs report `torch.cuda.is_available()==True` yet can't run kernels because their arch isn't in the installed PyTorch build (e.g. RTX 5060 Ti `sm_120` vs a torch built for `sm<=90` → "no kernel image available"). `app/main.py` calls `disable_cuda_if_unsupported()` (`app/utils/device.py`) at startup: when the arch isn't usable it overrides `torch.cuda.is_available` to False **process-wide**, so SAM3's internal hardcoded cuda checks, Ultralytics, and our own device picks all fall back to CPU. Tracking/SAM3 still run, just slower — don't treat CPU mode as an error. (`tracking.py` device selection uses `cuda_is_usable()` from the same util; the older experiment-only YOLO probe `_probe_inference_device` in `live_experiment.py`, commit `0e4e33a`, still exists but the startup hide now also covers the tracking + SAM3 paths.)
