...
- **Backend:** Python 3.11, FastAPI, Pydantic, PyTorch 2.8.0 (CUDA 12.8 / cu128 — required for the RTX 50-series sm_120 GPU), Ultralytics 8.3.102 (YOLO), OpenCV, ffmpeg/ffprobe. Optional SAM3 support (see Domain notes).
...
- **SAM3 (optional):** `app/routers/tracking.py` adds `temp/models/` to `sys.path` and tries `from sam3.model_builder import build_sam3_video_model`. If the package isn't dropped into `temp/models/sam3/`, `SAM3_AVAILABLE` is False and SAM3-specific endpoints degrade — this is expected, not a bug. **Runtime deps** for the dropped-in package (not on PyPI) are pinned in the `sam3` extra — install with `uv pip install -e '.[sam3]'` from `pymice/backend/`. **Critical pin:** SAM3's `model_builder` does `import pkg_resources`, which ships only with **`setuptools<81`** (newer setuptools removed it) — don't unpin or the import silently fails → `SAM3_AVAILABLE=False`. On a tracking run SAM3 writes, per frame, the centroid (trajectory) plus a **simplified mask polygon** (`cv2.approxPolyDP`, ~1% epsilon — not the full pixel mask) into the results JSON with `detection_method="sam3"`.
...
- **GPU fallback / unsupported-arch hide:** some GPUs report `torch.cuda.is_available()==True` yet can't run kernels because their compute capability isn't in the installed PyTorch build's `get_arch_list()`. `app/main.py` calls `disable_cuda_if_unsupported()` (`app/utils/device.py`) at startup: when the arch isn't usable it overrides `torch.cuda.is_available` to False **process-wide**, so SAM3's internal hardcoded cuda checks, Ultralytics, and our device picks all fall back to CPU (don't treat CPU mode as an error). It's a **no-op when the GPU is usable**, and `tracking.py` device selection uses `cuda_is_usable()` from the same util. **This machine's RTX 5060 Ti is `sm_120`:** under torch 2.6+cu124 (`sm<=90`) it hit "no kernel image available" → CPU (~60s/frame for SAM3); **torch 2.8.0+cu128 includes `sm_120`**, so the GPU is now used (~1s/frame for SAM3, the probe returns usable, the hide is a no-op). The util is forward-compatible — it auto-uses any GPU whose arch the torch build supports, no code change. (The older experiment-only YOLO probe `_probe_inference_device` in `live_experiment.py`, commit `0e4e33a`, still exists; the startup hide also covers the tracking + SAM3 paths.)
